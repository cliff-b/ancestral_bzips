...
setwd("/Users/Cliff/Documents/Kosuri Lab/R and python/ancestral tree/data_for_R2")
...
anctodiv2 <- select(anctodiv2, xpep, ypep, 'distance' = distance.x, ancints, pearsonlog, ancestor, strength)
...
wilcox.test(filter(ancsspecs, rel == 'paralogs', strength == 'strong')$pearsonlog, filter(ancsspecs, rel == 'ancdesc', strength == 'strong')$pearsonlog)

wilcox.test(filter(ancsspecs, rel == 'paralogs', strength == 'weak')$pearsonlog, filter(ancsspecs, rel == 'ancdesc', strength == 'weak')$pearsonlog)
...
colnames(numpars) <- c('xpep','ypep','ancestor','distance', 'numpars')
colnames(numparsweak) <- c('xpep','ypep','ancestor','distance', 'numpars')
ancnums <- full_join(numpars, anctodiv, by = c('xpep','ypep','distance'))
ancnumsweak <- full_join(numparsweak, anctodivweak, by = c('xpep','ypep','distance'))

ancs <- filter(disttointprof, xpep %in% ancnums$ancestor | ypep %in% ancnums$ancestor)
desc1 <- filter(disttointprof, xpep %in% ancnums$xpep | ypep %in% ancnums$xpep)
desc2 <- filter(disttointprof, xpep %in% ancnums$ypep | ypep %in% ancnums$ypep)
...
sumallspecsgain <- group_by(allspecsgain, age) %>%
  summarise('strongs' = sum(intscore == 1.0), 'weaks' = sum(intscore == 0.5), 'nons' = sum(intscore == 0.0))
sumallspecsgain <- gather(sumallspecsgain, key = 'intstrength', value = 'count', -age)
sumallspecsgain$intstrength <- factor(sumallspecsgain$intstrength, levels = c( "strongs","weaks", "nons"))
g.sumallspecs <- ggplot(sumallspecsgain, aes(age,count, fill=intstrength)) + geom_col(position = 'fill') + theme_cowplot() +
  scale_x_discrete(label = c('Ancestors','Descendants')) + labs(x = '', y ='Fraction', title = '210206 Percent of interactions by ancestor and specgains') +
  scale_fill_manual(name = 'Classification\nScore', values = c("#FE3131","#C300EC","#0090F3"), labels = c('Interaction','Weak interaction','Non-interaction'))
g.sumallspecs

ggsave(g.sumallspecs, file= '../figures/210206 Interactions percentages by ancdes.pdf', scale = 0.6)

ancsweak <- filter(disttointprof, xpep %in% ancnumsweak$ancestor | ypep %in% ancnumsweak$ancestor)
desc1weak <- filter(disttointprof, xpep %in% ancnumsweak$xpep | ypep %in% ancnumsweak$xpep)
desc2weak <- filter(disttointprof, xpep %in% ancnumsweak$ypep | ypep %in% ancnumsweak$ypep)

ancsweak <- data.frame(ancsweak, 'age' = 'ancestor')
desc1weak <- data.frame(desc1weak, 'age' = 'leg1')
desc2weak <- data.frame(desc2weak, 'age' = 'leg1')

allspecsgainweak <- bind_rows(ancsweak, desc1weak, desc2weak)
sumallspecsgainweak <- group_by(allspecsgainweak, age) %>%
  summarise('strongs' = sum(intscore == 1.0), 'weaks' = sum(intscore == 0.5), 'nons' = sum(intscore == 0.0))
sumallspecsgainweak <- gather(sumallspecsgainweak, key = 'intstrength', value = 'count', -age)
sumallspecsgainweak$intstrength <- factor(sumallspecsgainweak$intstrength, levels = c( "strongs","weaks", "nons"))
g.sumallspecsweak <- ggplot(sumallspecsgainweak, aes(age,count, fill=intstrength)) + geom_col(position = 'fill') + theme_cowplot() +
  scale_x_discrete(label = c('Ancestors','Descendants')) + labs(x = '', y ='Fraction', title = '210206 Percent of interactions by ancestor and specgains weak') +
  scale_fill_manual(name = 'Classification\nScore', values = c("#FE3131","#C300EC","#0090F3"), labels = c('Interaction','Weak interaction','Non-interaction'))
g.sumallspecsweak

ggsave(g.sumallspecsweak, file= '../figures/210206 Interactions percentages by ancdes weak.pdf', scale = 0.6)
...
o <- read.csv('201208_orthosandparasweakintsasints.csv')
orthovspara <- read.csv('201208_orthosandparasweakintsasints.csv')
orthovsparaweak <- read.csv('201208_orthosandparasweakintsasloss.csv')
...
orthostrong <- filter(sumorthosvspara, strength == 'strong')
orthostrong <- select(orthostrong, bin_avg, logtype, meanscore)
orthostrong <- pivot_wider(orthostrong, names_from = logtype, values_from = meanscore)

orthoweak <- filter(sumorthosvspara, strength == 'weak')
orthoweak <- select(orthoweak, bin_avg, logtype, meanscore)
orthoweak <- pivot_wider(orthoweak, names_from = logtype, values_from = meanscore)
...
orthonames <- read.csv('210208_filtered_orthosonly.csv')
...
regains <- read.csv(file = "../data_for_R2/210205_Regains.csv", strip.white = T)
regains$int_subdivision[regains$int_subdivision == "trace_w_n2desc"] <- "trace_w_n1desc"

sumregs <- group_by(regains, Dupe_node, int_subdivision, interaction) %>%
  summarise(counts =  n(), mdist = mean(distance))


sumallnodes <- group_by(regains, int_subdivision, interaction) %>%
  summarise(counts = n(), mdist = mean(distance), )

g.sumallnodes <- ggplot(sumallnodes, aes(int_subdivision, counts, fill = interaction)) + geom_col(position = "dodge") + scale_x_discrete(labels = c("All descendants", "Time-matched descendants", "Descendants with\npre-specificity gain nodes")) + 
  scale_fill_manual(values = c("#00A08A", "#F2AD00", "#FF0000"), name = "Interaction?", labels = c("No", "Weak", "Yes")) + labs(x = "Interaction partners", y = "Count", title = "200129 EA66k post-specificity gain interactions") 
g.sumallnodes

g.dists <- ggplot(filter(regains, int_subdivision != "trace_w_n1desc"), aes(as.character(Dupe_node), fill = interaction)) + geom_bar(stat = "count", position = "fill")
g.dists

regains$int_subdivision <- factor(regains$int_subdivision, levels = rev(c("match_desc", "all_desc", "trace_w_n2desc", "trace_w_n1desc")))
g.bars <- ggplot(regains, aes(int_subdivision, fill = interaction)) + geom_bar(stat = "count", position = "fill") + labs(x = "Interaction Type", y = "Fraction", title = "210205 EA66k Post-specificity odds of reinteracting") +
  scale_fill_manual(name = "Classification\nscore", labels = c("Non-interaction", "Weak  interaction", "Interaction"), values = c("#0090F3","#C300EC","#FE3131")) + scale_x_discrete(labels = c("Descendants with\npre-specificity gain nodes","All descendants", "Paralogs")) + 
  theme_cowplot() + theme(text = element_text(family = "Myriad Web Pro", size=14))
g.bars

ggsave(g.bars, file = '../figures/210205 fraction of ints regained.pdf', scale = 0.6)



regainsweaks <- read.csv(file = "../data_for_R2/210205_Regainswithweakloss.csv", strip.white = T)
regainsweaks$int_subdivision[regainsweaks$int_subdivision == "trace_w_n2desc"] <- "trace_w_n1desc"

sumregsewak <- group_by(regainsweaks, Dupe_node, int_subdivision, interaction) %>%
  summarise(counts =  n(), mdist = mean(distance))


sumallnodesweak <- group_by(regainsweaks, int_subdivision, interaction) %>%
  summarise(counts = n(), mdist = mean(distance), )

g.sumallnodes <- ggplot(sumallnodesweak, aes(int_subdivision, counts, fill = interaction)) + geom_col(position = "dodge") + scale_x_discrete(labels = c("All descendants", "Time-matched descendants", "Descendants with\npre-specificity gain nodes")) + 
  scale_fill_manual(values = c("#00A08A", "#F2AD00", "#FF0000"), name = "Interaction?", labels = c("No", "Weak", "Yes")) + labs(x = "Interaction partners", y = "Count", title = "200129 EA66k post-specificity gain interactions") 
g.sumallnodes

g.dists <- ggplot(filter(regainsweaks, int_subdivision != "trace_w_n1desc"), aes(as.character(Dupe_node), fill = interaction)) + geom_bar(stat = "count", position = "fill")
g.dists

regainsweaks$int_subdivision <- factor(regainsweaks$int_subdivision, levels = rev(c("match_desc", "all_desc", "trace_w_n2desc", "trace_w_n1desc")))
g.bars <- ggplot(regainsweaks, aes(int_subdivision, fill = interaction)) + geom_bar(stat = "count", position = "fill") + labs(x = "Interaction Type", y = "Fraction", title = "210205 EA66k Post-specificity odds of reinteracting") +
  scale_fill_manual(name = "Classification\nscore", labels = c("Non-interaction", "Weak  interaction", "Interaction"), values = c("#0090F3","#C300EC","#FE3131")) + scale_x_discrete(labels = c("Descendants with\npre-specificity gain nodes","All descendants", "Paralogs")) + 
  theme_cowplot() + theme(text = element_text(family = "Myriad Web Pro", size=14))
g.bars

ggsave(g.bars, file = '../figures/210205 fraction of ints regainedweaks.pdf', scale = 0.6)
